...
void Adv_Diff();	//calculate advection and diffusion terms for the momentum equations
void mom();	//solve the momentum equations
void press_corr();	//pressure correction procedure
void solve(int file);	//solution algorithm
int continuity();	//check divergence of the flow
int steady(int t);	//check whether steady state is reached or not
...
{
	double N,E,W,S;
	for(int j=1;j<=J;j++)	//terms for x momentum equation
	{
		for(int i=1;i<I;i++)
		{
	...
	for(int j=1;j<J;j++)	//terms for y momentum equations
	{
		for(int i=1;i<=I;i++)
		{
...
void mom()
{
	Adv_Diff();	//calculate the advection and diffusion terms
...
int steady(int CNT)
{
	...
	for(int j=1;j<=J;j++)
		for(int i=1;i<=I;i++)
			RES+=abs(U[j][i]-U_prev[j][i])+abs(V[j][i]-V_prev[j][i])+abs(P[j][i]-P_prev[j][i]);
	if(CNT%100==0) cout<<"RES_steady = "<<abs(RES)<<endl;
	if(abs(RES)<=10*EPS) return 0;
	else return 1;
}
